...
export async function main(event) {
  return new Promise(async (resolve, reject) => {
    ...
    const params = {
      ...
      ExpressionAttributeNames: dynamoDbUtils.getExpressionAttributeNames(appointment),
      ExpressionAttributeValues: dynamoDbUtils.getExpressionAttributeValues(appointment),
    ...
    try {
      ...
      resolve(success({ status: 'appointment updated successfully', appointment }));
    } catch (e) {
      console.error('server error updating the appointment: ', e.response);
      reject(serverFailure('Server error updating the appointment', e.response));
    }
  });
}
